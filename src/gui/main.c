...
int main(void)
{
    ...
    rc = vault_load(&v, vault_path(), NULL);
    if (rc == VAULT_ERR_LOCKED) {
        Printf((CONST_STRPTR)"AmiAuth: this vault is encrypted; unlock arrives in a later "
               "GUI stage. Use an always-unlocked vault for now.\n");
        close_libs();
        return 5;
    }
    if (rc != VAULT_OK) {
